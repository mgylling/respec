...
export function run(conf, doc, cb) {
  ...
  var release = toElement(
  `<div class='subtitle'>${conf.specStatus}<br/>Version ${conf.specVersion}</div>`);
  header.appendChild(release);

  var statusPD = toElement(
  `<span class='statusPD' data-content="${conf.specStatus}">${conf.specStatus}</span>`);
  header.appendChild(statusPD);

  var versionTable = toElement(
