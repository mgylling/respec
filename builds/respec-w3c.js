  ...
  function htmlJoinAnd(array, mapper = item => item) {
    ...
    switch (items.length) {
      ...
      case 1: // "x"
        return items[0];
      case 2: // x and y
        return html$1`${items[0]}${l10n$2.x_and_y}${items[1]}`;
      default: {
        const joined = htmlJoinComma(items.slice(0, -1));
        return html$1`${joined}${l10n$2.x_y_and_z}${items[items.length - 1]}`;
      }
    }
  }
...
>
  ...
  // @ts-check
  const name$k = "w3c/l10n";
  const additions = {
    en: {
      status_at_publication: html$1`This section describes the status of this
    ...
    ko: {
      status_at_publication: html$1`이 부분은 현재 문서의 발행 당시 상태에 대해
    ...
    zh: {
      status_at_publication: html$1`本章节描述了本文档的发布状态。其它更新版本可能会覆盖本文档。W3C的文档列
      表和最新版本可通过<a href="https://www.w3.org/TR/">W3C技术报告</a
      >索引访问。`,
    },
    ja: {
      status_at_publication: html$1`この節には、公開時点でのこの文書の位置づけが記されている。他の文書によって置き換えられる可能性がある。現時点でのW3Cの発行文書とこのテクニカルレポートの最新版は、下記から参照できる。
      <a href="https://www.w3.org/TR/">W3C technical reports index</a>
      (https://www.w3.org/TR/)`,
    },
    es: {
      status_at_publication: html$1`Esta sección describe el estado del presente
    ...
    de: {
      status_at_publication: html$1`Dieser Abschnitt beschreibt den Status des
  ...
  const l10n$6 = getIntlData(localizationStrings$3);
  ...
  function run$b(conf) {
    ...
    h1Elem.append(` ${l10n$6.level} ${level}`);
    document.title = `${document.title} ${l10n$6.level} ${level}`;
  ...
  function showLinkData(data) {
    return html$1`<dd class="${data.class ? data.class : null}">
    ${data.href
  ...
  const localizationStrings$4 = {
    en: {
      until(date) {
        return html$1` Until ${date} `;
      },
    },
    es: {
      until(date) {
        return html$1` Hasta ${date} `;
      },
    },
    ko: {
      until(date) {
        return html$1` ${date} 이전 `;
      },
    },
    ja: {
      until(date) {
        return html$1` ${date} 以前 `;
      },
    },
    de: {
      until(date) {
        return html$1` bis ${date} `;
      },
    },
    zh: {
      until(date) {
        return html$1` 直到 ${date} `;
      },
    },
  };

  const lang$a = lang in localizationStrings$4 ? lang : "en";

  var showPeople = (items = []) => {
    const l10n = localizationStrings$4[lang$a];
    ...
    ></dd>`;
      ...
      >`);
        ...
        contents.push(
          html$1`<a class="u-url url p-name fn" href="${p.url}">${personName}</a>`
        );
      } else {
      ...
      if (p.orcid) {
        contents.push(
          html$1`<a class="p-name orcid" href="${p.orcid}"
          ><svg
            width="16"
            height="16"
      ...
      if (p.retiredDate) {
        ...
        contents.push(html$1` - ${l10n.until(timeElem)} `);
      }
  ...
  var headersTmpl = (conf, options) => {
    ...
    <dl>
      ${!conf.isNoTrack
        ? html$1`
            <dt>${l10n$7.this_version}</dt>
            <dd>
              <a class="u-url" href="${conf.thisVersion}"
                >${conf.thisVersion}</a
              >
            </dd>
            <dt>${l10n$7.latest_published_version}</dt>
            <dd>
              ${conf.latestVersion
                ? html$1`<a href="${conf.latestVersion}"
      ...
      ${conf.edDraftURI
        ? html$1`
            <dt>${l10n$7.latest_editors_draft}</dt>
            <dd><a href="${conf.edDraftURI}">${conf.edDraftURI}</a></dd>
          `
        : ""}
      ${conf.testSuiteURI
        ? html$1`
            <dt>${l10n$7.test_suite}</dt>
            <dd><a href="${conf.testSuiteURI}">${conf.testSuiteURI}</a></dd>
          `
        : ""}
      ${conf.implementationReportURI
        ? html$1`
            <dt>${l10n$7.implementation_report}</dt>
            <dd>
      ...
      ${conf.isED && conf.prevED
        ? html$1`
            <dt>${l10n$7.prev_editor_draft}</dt>
            <dd><a href="${conf.prevED}">${conf.prevED}</a></dd>
          `
        : ""}
      ${conf.showPreviousVersion
        ? html$1`
            <dt>${l10n$7.prev_version}</dt>
      ...
      ${!conf.prevRecURI
        ...
        : conf.isRec
        ? html$1`
            <dt>${l10n$7.prev_recommendation}</dt>
            <dd><a href="${conf.prevRecURI}">${conf.prevRecURI}</a></dd>
          `
        : html$1`
            <dt>${l10n$7.latest_recommendation}</dt>
            <dd><a href="${conf.prevRecURI}">${conf.prevRecURI}</a></dd>
          `}
      <dt>${conf.multipleEditors ? l10n$7.editors : l10n$7.editor}</dt>
      ${showPeople(conf.editors)}
      ${Array.isArray(conf.formerEditors) && conf.formerEditors.length > 0
        ? html$1`
            <dt>
              ${conf.multipleFormerEditors
                ? l10n$7.former_editors
                : l10n$7.former_editor}
            </dt>
      ...
      ${conf.authors
        ? html$1`
            <dt>
              ${conf.multipleAuthors ? l10n$7.authors : l10n$7.author}
            </dt>
  ...
  function inPlaceModificationDate(date) {
    ...
    return html$1`<time class="dt-modified" datetime="${date}"
  ...
  function renderOfficialCopyright(conf) {
    return html$1`<p class="copyright">
  ...
  var cgbgHeadersTmpl = (conf, options) => {
    ...
    ${conf.subtitle ? html$1`<h2 id="subtitle">${conf.subtitle}</h2>` : ""}
    <h2>
      ${conf.longStatus}
      <time class="dt-published" datetime="${conf.dashDate}"
        >${conf.publishHumanDate}</time
  ...
  function renderPreview(conf) {
    ...
    return html$1`<details class="annoying-warning" open="">
    <summary
      >This is a
  ...
  function renderDeliverer(conf) {
    ...
    const wontBeRec = recNotExpected
      ...
      : "";
    return html$1`<p data-deliverer="${isNote || isIGNote ? wgId : null}">
    ${producers} ${wontBeRec}
    ${!isNote && !isIGNote
      ? html$1`
          ${multipleWGs
            ? html$1` W3C maintains ${wgPatentHTML} `
            : html$1`
                W3C maintains a
  ...
  function renderPublicList(conf, opts) {
    ...
    <a href="${archivesURL}">archives</a>)${subjectPrefix
      ? html$1` with <code>${subjectPrefix}</code> at the start of your email's
          subject`
      : ""}.
  </p>`;
  }
  ...
  var cgbgSotdTmpl = (conf, opts) => {
    ...
    <h2>${l10n$8.sotd}</h2>
    ${conf.isPreview
      ? html$1`<details class="annoying-warning" open="">
          <summary
            >This is a
  ...
  function run$d(conf) {
    ...
    if (Array.isArray(conf.wg)) {
      ...
      conf.wgHTML = htmlJoinAnd(conf.wg, (wg, idx) => {
        return html$1`the <a href="${conf.wgURI[idx]}">${wg}</a>`;
      });
      const pats = [];
      ...
      if (conf.wg) {
        conf.wgHTML = html$1`the <a href="${conf.wgURI}">${conf.wg}</a>`;
      }
    }
  ...
  async function run$e() {
    ...
    abstractHeading.textContent = l10n$9.abstract;
  ...
  const name$s = "core/data-transform";
  ...
  const name$v = "core/render-biblio";
  ...
  function run$h(conf) {
    ...
    if (!document.querySelector("section#references > h2")) {
      refSection.prepend(html$1`<h2>${l10n$a.references}</h2>`);
    }

    refSection.classList.add("appendix");

    if (norms.length) {
      const sec = createReferencesSection(norms, l10n$a.norm_references);
      refSection.appendChild(sec);
    }
    if (informs.length) {
      const sec = createReferencesSection(informs, l10n$a.info_references);
      refSection.appendChild(sec);
    }
  ...
  function run$i(conf) {
    ...
    const keywords = l10n$b.rfc2119Keywords();
    const rx = new RegExp(
      `(${[
      keywords.source,
  ...
  function processConformance(conformance, conf) {
    ...
    const keywords = htmlJoinAnd(
      terms.sort(),
      item => html$1`<em class="rfc2119">${item}</em>`
    );
    const plural = terms.length > 1;
    const content = html$1`
    <h2>${l10n$c.conformance}</h2>
    <p>
      ${l10n$c.normativity}
    </p>
    ${terms.length ? l10n$c.keywordInterpretation(keywords, plural) : null}
  `;
  ...
  function makeIssueSectionSummary(issueList) {
    ...
    issueList.hasChildNodes()
      ...
      : issueSummaryElement.append(html$1`<p>${l10n$e.no_issues_in_spec}</p>`);
    if (
      !heading ||
  ...
  const lang$b = lang in localizationStrings$d ? lang : "en";
  ...
  function run$o() {
    ...
    [...bps].forEach((bp, num) => {
      ...
      ><bdi lang="${lang$b}">${l10n$f.best_practice}${num + 1}</bdi></a
    >`;
  ...
  function run$p() {
    ...
    if (tof.length && tofElement) {
      ...
      tofElement.append(
        html$1`<h2>${l10n$g.list_of_figures}</h2>`,
        html$1`<ul class="tof">
        ${tof}
      </ul>`
      );
  ...
  function decorateFigure(figure, caption, i) {
    ...
    caption.prepend(l10n$g.fig, html$1`<bdi class="figno">${i + 1}</bdi>`, " ");
  }
   ...
   *
   */
  const name$E = "core/webidl-clipboard";
  ...
  function mapTitleToDfns() {
    ...
    for (const key of definitionMap.keys()) {
      ...
      if (duplicates.length > 0) {
        showInlineError(duplicates, l10n$h.duplicateMsg(key), l10n$h.duplicateTitle);
      }
    }
    return titleToDfns;
  }
   ...
   * that is preferred.
   */
  const name$J = "core/webidl-index";
  ...
  function renderLocalTerm(term, dfns) {
    ...
    <ul>
      ${dfns.map(dfn => {
        const type = getLocalTermType(dfn);
        const text = getLocalTermSuffix(dfn, type, term) || l10n$i.dfnOf;
  ...
  function createTableOfContents(ol) {
    ...
    const h2 = html$1`<h2 class="introductory">${l10n$j.toc}</h2>`;
    addId(h2);
    nav.append(h2, ol);
    const ref =
  ...
  function run$z() {
    Array.from(document.querySelectorAll("section.informative"))
      ...
      .forEach(heading => {
        heading.after(html$1`<p><em>${l10n$k.informative}</em></p>`);
      });
  }
  ...
  async function run$C(conf) {
    ...
    const style = document.createElement("style");
    style.textContent = await loadStyle$7();
  ...
  function getEngineSupportIcons(engines) {
    if (engines.length === 3) {
      return html$1`<span title="${l10n$l.inAllEngines}">✅</span>`;
    }
    if (engines.length < 2) {
      return html$1`<span title="${l10n$l.inSomeEngines}">🚫</span>`;
    }
  ...
  function getEngineSupport(engines) {
    if (engines.length === 3) {
      return html$1`<p class="engines-all">${l10n$l.inAllEngines}</p>`;
    }
    if (engines.length < 2) {
      return html$1`<p class="engines-some">${l10n$l.inSomeEngines}</p>`;
    }
  }
  ...
  const saveDialog = {
    async show(button) {
      ...
      ui.freshModal(l10n$m.save_snapshot, div, button);
    },
  };
  ...
  let button;
  if (supportsDownload) {
    button = ui.addCommand(l10n$m.save_snapshot, show, "Ctrl+Shift+Alt+S", "💾");
  }
  ...
  const l10n$n = getIntlData(localizationStrings$l);

  const button$1 = ui.addCommand(
    l10n$n.search_specref,
    show$1,
  ...
  function show$1() {
    render();
    ui.freshModal(l10n$n.search_specref, form, button$1);
    /** @type {HTMLElement} */
...
`;
  ...
  const lang$c = lang in localizationStrings$m ? lang : "en";
  const l10n$o = localizationStrings$m[lang$c];

  const button$2 = ui.addCommand(l10n$o.title, show$2, "Ctrl+Shift+Alt+x", "📚");
  ...
  `;
    ui.freshModal(l10n$o.title, xrefSearchUI, button$2);
  }
  ...
  const l10n$p = getIntlData(localizationStrings$n);

  const button$3 = ui.addCommand(
    l10n$p.definition_list,
    show$3,
    "Ctrl+Shift+Alt+D",
    "📔"
  );
  ...
  function show$3() {
    ...
    ui.freshModal(l10n$p.list_of_definitions, ul, button$3);
  }
  ...
  const l10n$q = getIntlData(localizationStrings$o);
  ...
  const button$4 = ui.addCommand(
    `${l10n$q.about_respec} ${window.respecVersion}`,
    show$4,
    "Ctrl+Shift+Alt+A",
    "ℹ️"
  );
...
`;
    ui.freshModal(`${l10n$q.about_respec} - ${window.respecVersion}`, div, button$4);
  }
  ...
  const l10n$r = getIntlData(localizationStrings$p);

  const name$X = "core/data-tests";
  ...
  function run$G(conf) {
    ...
    if (!conf.testSuiteURI) {
      pub("error", l10n$r.missing_test_suite_uri);
      return;
    }
  ...
  const name$Z = "core/highlight-vars";
  ...
  const name$_ = "core/dfn-panel";

  async function run$J() {
    const css = await loadStyle$a();
    document.head.insertBefore(
      html$1`<style>
      ${css}
    </style>`,
  ...
  const tooltipStylePromise = loadStyle$b();
  ...
  const name$10 = "core/algorithms";

  const cssPromise$3 = loadStyle$c();
  ...
  const name$11 = "core/anchor-expander";
  ...
  const name$13 = "core/custom-elements/index";
